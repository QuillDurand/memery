...
def get_grid(filepaths, n=4):
    ...
    grid = widgets.GridBox(imgs, layout=widgets.Layout(grid_template_columns="repeat(auto-fit)"))
    return grid
    ...
    if slug not in searches.keys():
        with logbox:
            ...
            if image_query:
                ...
                img = [Image.open(BytesIO(file_info['content'])).convert('RGB') for name, file_info in
                       image_query.items()]
...
class AppPage:
    ...
    def __init__(self):
        ...
        self.all_inputs_layout = widgets.Layout(max_width='80vw', min_height='40vh', flex_flow='row wrap',
                                                align_content='flex-start')

        self.inputs = widgets.Box(
            [self.path, self.query, self.image_query, self.n_images, self.go, self.im_display_zone, self.logbox],
            layout=self.all_inputs_layout)
    ...
    def page_update(self, b):
        update_tabs(self.path, self.query, self.n_images, self.searches, self.tabs, self.logbox, self.im_display_zone,
                    self.image_query.value)
